# DEVELOPER GUIDE - WindowsOrchestrator 1.72
...
2. [System Architecture and Security Model](#2-system-architecture-and-security-model)
    ...
    2.3. [Temporal Orchestration and Parallelism](#23-temporal-orchestration-and-parallelism)
        2.3.1. [Backup/Close Decoupling](#231-backupclose-decoupling)
        2.3.2. [Typical Daily Chronology (Workflow)](#232-typical-daily-chronology-workflow)
...
4. [Code Structure and Component Analysis](#4-code-structure-and-component-analysis)
    ...
    4.2. [The Central Module: WindowsOrchestratorUtils.psm1](#42-the-central-module-windowsorchestratorutilspsm1)
        ...
        4.2.2. [Internationalization System (i18n)](#422-internationalization-system-i18n)
            [Localization Strategy (v1.72+)](#localization-strategy-v172)
...
## 2.3. Temporal Orchestration and Parallelism

The orchestrator does not rely on a single script that "sleeps" (loop `Start-Sleep`) waiting for an action time. It relies on the scheduler to trigger punctual and independent actions.

### 2.3.1. Backup/Close Decoupling
...
### 2.3.2. Typical Daily Chronology (Workflow)
...
| **Interactive (LogonType)** | Specific type of scheduled task that executes **in** the logged-in user's session. It is the cornerstone of version 1.72's architecture, allowing graphical application launch without knowing the user's password. |
...
This project (v1.72) is the result of a hybrid Human-AI collaboration:

*   **Ronan Davalan**: Project Manager, Principal Architect, Quality Assurance (QA).
*   **Google Gemini**: AI Architect, Planner, Technical Writer.
*   **Grok**: AI Developer (Implementation).
*   **Claude**: AI Technical Consultant (Code Review & P/Invoke Solutions).

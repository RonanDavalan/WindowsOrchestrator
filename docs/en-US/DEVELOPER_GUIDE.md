...
4. [Code Structure and Component Analysis](#4-code-structure-and-component-analysis)
    ...
    4.6. [Specialized Modules](#46-specialized-modules)
        4.6.1. [Invoke-DatabaseBackup.ps1: Autonomous Backup](#461-invoke-databasebackupps1-autonomous-backup)
            ...
            [D. Watchdog Loop and MonitorTimeout](#d-watchdog-loop-and-monitortimeout)
...
*   **`Standard`**:
    *   **Technical Action**: Does **not** modify the `AutoAdminLogon` registry key. The orchestrator applies a non-interference principle: the existing registry state is preserved as-is.
...
The retention management relies on the file system's `LastWriteTime` metadata and a consistent naming convention.

*   **Naming Format**: Files generated by the orchestrator follow the pattern: `BaseName_yyyy-MM-dd.ext` (e.g., `database_2025-03-15.db`).
*   **Algorithm**:
    1.  The script lists files in `DatabaseDestinationPath`.
    2.  It filters files whose `LastWriteTime` is older than `(Get-Date).AddDays(-$DatabaseKeepDays)`.
    3.  Files matching this condition are deleted via `Remove-Item -Force`.
...
02:57:00 ─┼─ BEGIN TASK: WindowsOrchestrator-SystemBackup
          ...
          │  Action: Differential backup of modified files (< 24h).
          │
...
| :--- | :--- | :--- |
| **Registry Key** | No modification (non-interference principle). | `HKLM\...\Winlogon` `AutoAdminLogon = "1"` |

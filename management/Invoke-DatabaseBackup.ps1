...
<#
.SYNOPSIS
    Script de maintenance et sauvegarde pour l'orchestrateur de pré-redémarrage.
.DESCRIPTION
    Ce script gère le cycle de fin de journée :
    1. Surveillance et fermeture de l'application (Watchdog).
    2. Maintenance des logs (Réduction) - v1.74.
    3. Sauvegarde des bases de données (Backup).
    4. Redémarrage enchaîné (si configuré).
.NOTES
...
function Invoke-SystemCycle {
    Write-Log -DefaultMessage "Starting system maintenance cycle..." -MessageId "Log_Backup_Starting"
    ...
    $enableLogReduction = Get-ConfigValue -Section "DatabaseBackup" -Key "EnableLogReduction" -Type ([bool]) -DefaultValue $false

    # Si aucune action n'est requise, on sort
    if (-not $enableBackup -and -not $enableLogReduction) {
        Write-Log -DefaultMessage "No maintenance tasks enabled (Backup=False, LogReduction=False). Skipping cycle."
        return
    }

    # --- 1. WATCHDOG : Assurance Fermeture ---
    # Cette étape est commune et obligatoire avant de toucher aux fichiers
            ...
            while ($processActive) {
                if ($elapsed -ge $monitorTimeout) {
                    if ($forceKill) {
                        ...
                        Add-Error -DefaultErrorMessage "Process '{0}' still running after timeout ({1}s). Process aborted." -ErrorId "Error_Backup_TimeoutNoKill" -ErrorArgs $processToMonitor, $monitorTimeout
                        exit 1 # Arrêt d'urgence pour protéger les données
                    }
                }
    ...
    # --- 2. LOG MAINTENANCE (v1.74) ---
    if ($enableLogReduction) {
        Write-Log -DefaultMessage "Starting log reduction..." -MessageId "Log_LogReduction_Started"

        $logBaseDir = Get-ConfigValue -Section "DatabaseBackup" -Key "LogReductionBaseDir" -DefaultValue "..\.."
        $logFiles = Get-ConfigValue -Section "DatabaseBackup" -Key "LogReductionFiles" -DefaultValue ""
        $logLines = Get-ConfigValue -Section "DatabaseBackup" -Key "LogReductionLines" -Type ([int]) -DefaultValue 1000

        $scriptPath = Join-Path -Path $PSScriptRoot -ChildPath "..\reducelog.ps1"
        $scriptPath = [System.IO.Path]::GetFullPath($scriptPath)

        if (Test-Path $scriptPath) {
            try {
                # Appel du script externe et capture de sa sortie pour les logs
                $command = "& `"$scriptPath`" -BaseDir `"$logBaseDir`" -Files `"$logFiles`" -LinesToKeep $logLines"
                $output = Invoke-Expression $command

                # On loggue chaque ligne de sortie du script de réduction
                if ($output) {
                    foreach ($line in $output) {
                        Write-Log -DefaultMessage "LogReduction: $line" -Level "INFO"
                    }
                }
            } catch {
                Add-Error -DefaultErrorMessage "Log reduction failed: {0}" -ErrorId "Error_LogReduction_Failed" -ErrorArgs $_.Exception.Message
            }
        } else {
            Write-Log -DefaultMessage "Log reduction script not found at '{0}'." -MessageArgs $scriptPath -Level "WARN"
        }
    }
    ...
    if ($enableBackup) {
        ...
        if ($recentFiles.Count -eq 0) {
            Write-Log -DefaultMessage "No database files modified in the last 24 hours were found to back up." -MessageId "Log_Backup_NoFilesFound"
        } else {
...
# --- Enchaînement Système (Domino Effect) ---
...
try {
    # Lancement du cycle complet (Watchdog -> Logs -> Backup)
    Invoke-SystemCycle

    # Redémarrage si demandé par l'enchaînement (seulement après succès du cycle)
    if ($shouldReboot) {
    ...
    Add-Error -DefaultErrorMessage "Critical error during system cycle: {0}" -ErrorId "Error_Backup_Critical" -ErrorArgs $_.Exception.Message
}
